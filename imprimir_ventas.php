...
//consulta a la base de datos
    $ventas = consultas::get_datos("select * from v_factura_cab "
                    . "where ven_fecha between '" . $_REQUEST['vdesde'] . 
            "' and '" . $_REQUEST['vhasta'] . "' order by cod_factura");
    ...
    if (!empty($ventas)) {
        ...
        foreach ($ventas as $venta) {
            ...
            $pdf->SetFillColor(255, 255, 255);
            $pdf->SetFont('', '', 10);
            $pdf->Cell(20, 5, $venta['cod_factura'], 0, 0, 'C', 1);
            $pdf->Cell(70, 5, $venta['cliente'], 0, 0, 'C', 1);
            $pdf->Cell(30, 5, $venta['per_ci_ruc'], 0, 0, 'C', 1);
            ...
            $pdf->Cell(30, 5, $venta['nro_factura'], 0, 0, 'C', 1);
            $pdf->Cell(50, 5, $venta['ven_estado'], 0, 0, 'C', 1);
            $pdf->Ln(); //salto
//            
            $pdf->Ln();
            $pdf->SetFont('times', 'B', 9);
            $pdf->Cell(0, 3, 'DETALLE DE VENTA NRO.:    ' . $venta['cod_factura'], 0, 0, 'C', 0);
            $pdf->Ln();
           
            $detalles = consultas::get_datos("select * from v_det_factura_iva "
         . "where cod_factura=".$venta['cod_factura']." order by cod_factura");
           if (!empty($detalles)) {
...
//consulta a la base de datos
    $ventas = consultas::get_datos("select * from v_factura_cab "
                    . "where id_cliente=" . $_REQUEST['vcli'] . " order by cod_factura");
...
//consulta a la base de datos
    $ventas = consultas::get_datos("select * from v_factura_cab "
                    . "where ven_condicion='" . $_REQUEST['vcon'] . "' order by cod_factura");
...
//consulta a la base de datos
    $ventas = consultas::get_datos("select * from v_factura_cab "
                    . "where ven_estado='" . $_REQUEST['vesta'] . "' order by cod_factura");
...
//consulta a la base de datos
    $ventas = consultas::get_datos("select * from v_factura_cab "
                    . "where cod_usu=" . $_REQUEST['vusu'] . " order by cod_factura");
