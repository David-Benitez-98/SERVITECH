    ...
    <div class="col-md-6 col-md-offset-0">
        <div class="list-group">
            <a href="#" class="list-group-item active">
                Informes de Orden Compras 
            </a>              
        </div>   
        <div class="form-group col-md-12">
            <label class="col-sm-5 control-label">Presupuesto:</label>
            <div class="col-sm-6">
                <?php $ordencompras = consultas::get_datos(" select distinct estado_presu from v_presupuesto_compra where estado_presu = 'APROBADO'"); ?>                                                                 
                <select name="vpag" class="form-control select2" id="vorden">
                    <?php
                    if (!empty($ordencompras)) {
                        foreach ($ordencompras as $ordencompra) {
                            ?>
                            <option value="<?php echo $ordencompra['estado_presu']; ?>">
                                <?php echo $ordencompra['estado_presu']; ?></option>
                            <?php
                        }
                    } else {
                        ?>
...
<script>
    function enviar() {
        window.open("/servitech_tesis/imprimir_orden_compras.php?vorden=" + $('#vorden').val() + 
                '&vop=' + $('#op').val(), '_blank');
    }
</script>
