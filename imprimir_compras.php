...
//consulta a la base de datos
    $compras = consultas::get_datos("select * from v_compras "
            ...
            "' and '" . $_REQUEST['vhasta'] . "' order by cod_comp");
    ...
    if (!empty($compras)) {
        ...
        foreach ($compras as $compra) {
            ...
            $pdf->SetFillColor(255, 255, 255);
            $pdf->SetFont('', '', 10);
            $pdf->Cell(20, 5, $compra['cod_comp'], 0, 0, 'C', 1);
            ...
            $pdf->Cell(30, 5, $compra['com_estado'], 0, 0, 'C', 1);
            $pdf->Ln(); //salto
//            
            $pdf->Ln();
            $pdf->SetFont('times', 'B', 9);
            $pdf->Cell(0, 3, 'DETALLE DE COMPRA NRO.:    ' . $compra['cod_comp'], 0, 0, 'C', 0);
            $pdf->Ln();
           
            $detalles = consultas::get_datos("select * from v_detcompras "
         . "where cod_comp=".$compra['cod_comp']." order by cod_comp");
            if (!empty($detalles)) {
            ...
            foreach ($detalles as $detalle) {
                ...
                $pdf->Cell(50, 5, number_format($detalle['det_prec_comp'], 0, ',', '.'), 1, 0, 'C', 1);
...
//consulta a la base de datos
    $compras = consultas::get_datos("select * from v_compras "
                    . "where cod_prov=" . $_REQUEST['vprov'] . " order by cod_comp");
...
//consulta a la base de datos
    $compras = consultas::get_datos("select * from v_compras "
                    . "where com_condicion='" . $_REQUEST['vcon'] . "' order by cod_comp");
...
//consulta a la base de datos
    $compras = consultas::get_datos("select * from v_compras "
                    . "where com_estado='" . $_REQUEST['vesta'] . "' order by cod_comp");
...
//consulta a la base de datos
    $compras = consultas::get_datos("select * from v_compras "
                    . "where cod_usu=" . $_REQUEST['vusu'] . " order by cod_comp");
...
//consulta a la base de datos
    $compras = consultas::get_datos("select * from v_compras "
                    . "where estado_orden_compra='" . $_REQUEST['vorden'] . "' order by cod_comp");
    ...
    if (!empty($compras)) {
        ...
        foreach ($compras as $compra) {
            ...
            $pdf->Cell(50, 5, 'PROVEEDOR',0 , 0, 'C', 1);
            $pdf->Cell(30, 5, 'RUC O CI', 0, 0, 'C', 1);
            $pdf->Cell(25, 5, 'CONDICION',0 , 0, 'C', 1);
            $pdf->Cell(25, 5, 'TOTAL', 0, 0, 'C', 1);
            $pdf->Cell(30, 5, 'FECHA', 0, 0, 'C', 1);
            $pdf->Cell(30, 5, 'FACTURA', 0, 0, 'C', 1);
            $pdf->Cell(30, 5, 'ESTADO', 0, 0, 'C', 1);
            $pdf->Cell(15, 5, 'ORDEN', 0, 0, 'C', 1);
            ...
            $pdf->Cell(50, 5, $compra['persona'], 0, 0, 'C', 1);
            $pdf->Cell(30, 5, $compra['per_ci_ruc'], 0, 0, 'C', 1);
            $pdf->Cell(25, 5, $compra['com_condicion'], 0, 0, 'C', 1);
            $pdf->Cell(25, 5, number_format($compra['total'], 0, ',', '.'), 0, 0, 'C', 1);
            $pdf->Cell(30, 5, $compra['vfecha'], 0, 0, 'C', 1);
            $pdf->Cell(30, 5, $compra['nro_factura'], 0, 0, 'C', 1);
            $pdf->Cell(30, 5, $compra['com_estado'], 0, 0, 'C', 1);
            $pdf->Cell(10, 5, $compra['id_orden_compra'], 0, 0, 'C', 1);
