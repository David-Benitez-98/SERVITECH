...
//consulta a la base de datos
    $clientes = consultas::get_datos("select * from v_clientes "
         . "where cod_ciu=" . $_REQUEST['vciu'] . " order by id_cliente");
    if (!empty($clientes)) {
        foreach ($clientes as $cliente) {
            $pdf->Cell(20, 5, $cliente['id_cliente'], 1, 0, 'C', 1);
            $pdf->Cell(70, 5, $cliente['cliente'], 1, 0, 'L', 1);
            $pdf->Cell(30, 5, $cliente['per_ci_ruc'], 1, 0, 'L', 1);
            $pdf->Cell(30, 5, $cliente['per_telf'], 1, 0, 'L', 1);
            $pdf->Cell(80, 5, $cliente['per_direc'], 1, 0, 'L', 1);
            $pdf->Cell(40, 5, $cliente['dep_descri'], 1, 0, 'L', 1);
            $pdf->Cell(40, 5, $cliente['ciu_des'], 1, 0, 'L', 1);
            $pdf->Ln(); //salto
        }
    } else {
        $pdf->SetFont('times', 'B', '14');
...
if ($_REQUEST['vop'] == '2') {
 //consulta a la base de datos
    $clientes = consultas::get_datos("select * from v_clientes "
         . "where cod_depar=" . $_REQUEST['vdepar'] . " order by id_cliente");
    if (!empty($clientes)) {
        foreach ($clientes as $cliente) {
            $pdf->Cell(20, 5, $cliente['id_cliente'], 1, 0, 'C', 1);
            $pdf->Cell(70, 5, $cliente['cliente'], 1, 0, 'L', 1);
